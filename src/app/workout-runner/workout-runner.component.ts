...
export class WorkoutRunnerComponent implements OnInit {
  ...
  ngOnInit() {
    // create a workout
    this.workoutPlan = this.buildWorkoutPlan();

    // initialize the rest exercise
    ...
    this.exerciseTrackingInterval = window.setInterval(() => {
      if (this.exerciseRunningDuration >= this.currentExercise.duration) {
        ...
        if (this.currentExercise !== this.restExercise) {
          ...
          // storing the completed exercise
  ...
  // function to build workout plan
  buildWorkoutPlan(): WorkoutPlan {
    // create a workout plan - exercise plan w exercices
    ...
    workout.exercises.push(
      new ExercisePlan(
        new Exercise(
          "jumpingJacks",
          "Jumping Jacks",
          "A jumping jack or star jump",
          "JumpingJacks.png",
          "jumpingjacks.wav",
          // tslint:disable-next-line:max-line-length
          "Assume an erect position with feet together and arms at your side.<br>Slighty bend your knees and propel yourself at a few inches in the air.<br>",
          ["dmYwZH_BNd0", "BABOdJ-2Z6o", "c4DAnQ6DtF"]
        ),
        30
      )
    );
  ...
  // function to load next exercise
  getNextExercise(): ExercisePlan {
    let nextExercise: ExercisePlan = null;

    // pick the next one if on restExercise else set it to restExercise
    // unless done
    if (this.currentExercise === this.restExercise) {
  ...
  /**
   * Clears the setInterval and pause
   */
  pause() {
    clearInterval(this.exerciseTrackingInterval);
  ...
  /**
   * Coordinates the resume and pause function
   */
  pauseResumeToggle() {
  ...
  /**
   * Listen to keypress and toggles pause/resume
